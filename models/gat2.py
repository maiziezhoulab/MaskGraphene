...
class GAT(nn.Module):
    def __init__(self,
        ...
        # print("The layer dimension is: ", layer_dims)
        if num_layers + 1 < len(layer_dims):
            # print('The num_layers is: ', num_layers, 'The layer_dims is: ', layer_dims)
            layer_dims = layer_dims[:num_layers] + [layer_dims[-1]]
            # print('We change the layer dim to: ', layer_dims)

        if num_layers == 1:
            # print("The layer_dims[-1] // nhead: ", layer_dims[-1] // nhead)
            if not encoding:
                ...
                self.gat_layers.append(GATConv(
                    layer_dims[0], layer_dims[-1] // nhead, nhead_out,
                    feat_drop, attn_drop, negative_slope, last_residual, norm=last_norm, concat_out=concat_out))
        else:
            # input projection (no residual)
            self.gat_layers.append(GATConv(
                layer_dims[0], layer_dims[1] // nhead, nhead,
            ...
            for l in range(1, num_layers - 1):
                ...
                # print("The layer_dims[l+1] // nhead: ", layer_dims[l+1] // nhead)
                self.gat_layers.append(GATConv(
                    layer_dims[l], layer_dims[l + 1] // nhead, nhead,
                    feat_drop, attn_drop, negative_slope, residual, create_activation(activation), norm=norm,
                    concat_out=concat_out))
            ...
            if not encoding:
                ...
                # output projection
                self.gat_layers.append(GATConv(
                    layer_dims[-2], layer_dims[-1] // nhead, nhead_out,
                    feat_drop, attn_drop, negative_slope, last_residual, activation=last_activation, norm=last_norm,
                    concat_out=concat_out))
            # print("The layer_dims[-1] // nhead: ", layer_dims[-1] // nhead)
