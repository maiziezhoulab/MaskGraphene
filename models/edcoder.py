...
def setup_module(m_type, enc_dec, layer_info, num_layers, dropout, activation, residual, norm, nhead, nhead_out, attn_drop, negative_slope=0.2, concat_out=True, **kwargs) -> nn.Module:
...
class PreModel(nn.Module):
    def __init__(
            self,
            in_dim: int,
            num_hidden,  # int list [in_dim, num_hidden, out_dim] = hidden_dims
            num_layers: int,
            num_dec_layers: int,
            num_remasking: int,
            nhead: int,
        ...
        if encoder_type in ("gat",):
            ...
            enc_nhead = 1
        # [in_dim, num_hidden, out_dim] = num_hidden
        layer_info = [in_dim] + num_hidden

        dec_in_dim = num_hidden[-1]
        enc_num_hidden = num_hidden[-1]
        ...
        self.enc_mask_token = nn.Parameter(torch.zeros(1, layer_info[0]))
        self.dec_mask_token = nn.Parameter(torch.zeros(1, layer_info[-1]))
