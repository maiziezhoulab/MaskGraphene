...
#     mapping_mat = np.load(args.mapping_mat)
...
#     mapped_coords1 = Batch_list_new[1].obsm['spatial'][best_matches]
#     # Update the original obsm with the mapped coordinates
#     Batch_list_new[0].obsm['spatial'] = mapped_coords1
...
#     # TODO: update adj spatial
#     # adj_spatial = get_adj_mat_pi(Batch_list_new, mapping_mat, 7) # args.k

#     # print(adata_concat)
#     # print(non_zero_rate(adata_concat.X.toarray()))
#     # Cal_Spatial_Net(adata_concat, rad_cutoff=150)
...
#     adj_list = []
#     cls_ = 0
#     input_dir = '/maiziezhou_lab/yunfei/Projects/MaskGraphene_dev0625/aligned_coord_localOT/DLPFC151673_sim_data'
#     # for section_id in section_ids:
#     slice1 = sc.read_h5ad(os.path.join(input_dir, section_ids[0]))
#     slice1.X = slice1.X.toarray()
#     print(slice1.uns['spatial'])
#     # TODO
#     Cal_Spatial_Net(slice1, rad_cutoff=150)  # uns some feature shape cannot be concated
#     # adj_list.append(slice1.uns['adj'])
#     # print(slice1)
#     slice2 = sc.read_h5ad(os.path.join(input_dir, section_ids[1]))
#     slice2.X = slice2.X.toarray()
#     print(slice2.uns['spatial'])
#     Cal_Spatial_Net(slice2, rad_cutoff=150)
#     # slice2.X = slice2.X.astype(np.float32)
#     # adj_list.append(slice2.uns['adj'])
#     # print(slice2)
#     # ad_ = load_DLPFC(root_dir=args.st_data_dir, section_id=section_id)
#     # ad_.var_names_make_unique(join="++")
#     # print(mapping_mat.shape)
...
#     cls_ = max(cls_, len(slice1.obs['original_clusters'].unique()))

#     slice1.obs_names = [x + '_' + section_ids[0] for x in slice1.obs_names]
#     Batch_list_new.append(slice1)
    
#     slice2.obs_names = [x + '_' + section_ids[1] for x in slice2.obs_names]
#     Batch_list_new.append(slice2)
#     # print(len(Batch_list_new))
#     # print(align_coord.values)
...
#     adj_spatial = np.asarray(adata_concat.uns['adj'].todense())
#     # for batch_id in range(1,len(section_ids)):
#     #     adj_spatial = scipy.linalg.block_diag(adj_spatial, np.asarray(adj_list[batch_id].todense()))
